...
function showDeliveryError(fieldId, msg) {
  const field = document.getElementById(fieldId);
  if (!field) return;
  const group = field.closest('.form-group');
  const err   = document.getElementById(fieldId.replace('d-','d') + 'Error');
  if (group) group.classList.add('error');
  // find the span directly after input
  const errSpan = group ? group.querySelector('.form-error') : null;
  if (errSpan) { errSpan.textContent = msg; errSpan.classList.add('visible'); }
}

function clearDeliveryError(fieldId) {
  const field = document.getElementById(fieldId);
  if (!field) return;
  const group = field.closest('.form-group');
  if (group) {
...
['d-name','d-phone','d-address','d-order'].forEach(id => {
  const field = document.getElementById(id);
  if (field) field.addEventListener('input', () => clearDeliveryError(id));
});
...
if (deliveryForm) {
  deliveryForm.addEventListener('submit', e => {
    ...
    const dAddress = document.getElementById('d-address').value.trim();
    const dOrder   = document.getElementById('d-order').value.trim();
    const phoneRx  = /^[+]?[\d\s\-()]{8,16}$/;

    if (!dName || dName.length < 2) {
      showDeliveryError('d-name', 'Please enter your name.'); valid = false;
    } else clearDeliveryError('d-name');

    if (!dPhone || !phoneRx.test(dPhone)) {
      showDeliveryError('d-phone', 'Please enter a valid phone number.'); valid = false;
    } else clearDeliveryError('d-phone');

    if (!dAddress || dAddress.length < 8) {
      showDeliveryError('d-address', 'Please enter your full delivery address.'); valid = false;
    } else clearDeliveryError('d-address');

    if (!dOrder || dOrder.length < 5) {
      showDeliveryError('d-order', 'Please tell us what you would like to order.'); valid = false;
    } else clearDeliveryError('d-order');

    if (!valid) return;

    deliverySubmitBtn.disabled = true;
    const btnSpan = deliverySubmitBtn.querySelector('span');
    const origText = btnSpan.textContent;
    btnSpan.textContent = 'Placing Order…';
    deliverySubmitBtn.style.opacity = '0.7';

    setTimeout(() => {
      deliverySubmitBtn.disabled = false;
      btnSpan.textContent = origText;
      deliverySubmitBtn.style.opacity = '';
      if (confirmedPhone) confirmedPhone.textContent = dPhone;
      deliveryForm.reset();
      deliverySuccess.classList.add('visible');
      deliverySuccess.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
      setTimeout(() => deliverySuccess.classList.remove('visible'), 8000);
    }, 1800);
  });
}
