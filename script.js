...
const reserveConfirmedPhone = document.getElementById('reserveConfirmedPhone');
...
function showReserveError(fieldId, msg) {
...
function clearReserveError(fieldId) {
  const field = document.getElementById(fieldId);
  if (!field) return;
  const group = field.closest('.form-group');
  if (group) {
...
['r-name','r-phone','r-date','r-time','r-guests'].forEach(id => {
  const el = document.getElementById(id);
  if (el) el.addEventListener('input', () => clearReserveError(id));
  if (el) el.addEventListener('change', () => clearReserveError(id));
});
...
if (reserveForm) {
  reserveForm.addEventListener('submit', e => {
    ...
    const rGuests = document.getElementById('r-guests').value;
    const phoneRx = /^[+]?[\d\s\-()]{8,16}$/;

    if (!rName || rName.length < 2) {
      showReserveError('r-name', 'Please enter your full name.'); valid = false;
    } else clearReserveError('r-name');

    if (!rPhone || !phoneRx.test(rPhone)) {
      showReserveError('r-phone', 'Please enter a valid phone number.'); valid = false;
    } else clearReserveError('r-phone');

    if (!rDate) {
      showReserveError('r-date', 'Please select a date.'); valid = false;
    } else {
      const chosen = new Date(rDate);
      const today  = new Date(); today.setHours(0,0,0,0);
      if (chosen < today) {
        showReserveError('r-date', 'Please choose a future date.'); valid = false;
      } else clearReserveError('r-date');
    }

    if (!rTime) {
      showReserveError('r-time', 'Please select a time slot.'); valid = false;
    } else clearReserveError('r-time');

    if (!rGuests) {
      showReserveError('r-guests', 'Please select number of guests.'); valid = false;
    } else clearReserveError('r-guests');

    if (!valid) return;

    reserveSubmitBtn.disabled = true;
    const btnSpan = reserveSubmitBtn.querySelector('span');
    const origText = btnSpan.textContent;
    btnSpan.textContent = 'Confirming…';
    reserveSubmitBtn.style.opacity = '0.7';

    setTimeout(() => {
      reserveSubmitBtn.disabled = false;
      btnSpan.textContent = origText;
      reserveSubmitBtn.style.opacity = '';
      if (reserveConfirmedPhone) reserveConfirmedPhone.textContent = rPhone;
      reserveForm.reset();
      reserveSuccess.classList.add('visible');
      reserveSuccess.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
      setTimeout(() => reserveSuccess.classList.remove('visible'), 9000);
    }, 1800);
  });
}
...
function updateOpenStatus() {
  ...
  let opens, closes;
  if (day === 0)      { opens = 9;  closes = 21; }  // Sunday
  else if (day === 6) { opens = 8;  closes = 23; }  // Saturday
  else                { opens = 8;  closes = 22; }  // Mon–Fri

  const isOpen = hour >= opens && hour < closes;
  badge.className = 'reserve-open-badge ' + (isOpen ? 'open-now' : 'closed-now');

  if (isOpen) {
    const closingIn = closes - hour;
    if (closingIn <= 1) {
      statusEl.textContent = `Closing soon — ${Math.round(closingIn * 60)} mins left`;
    } else {
      statusEl.textContent = `Open now — closes at ${closes > 12 ? closes - 12 + ' PM' : closes + ' AM'}`;
    }
  } else {
    statusEl.textContent = `Closed now — opens at ${opens}:00 AM`;
  }
}
